  ...
  <Dialog>
   <DialogTrigger asChild>
    <motion.div
     ...
     className="group cursor-pointer p-6 rounded-2xl bg-white border border-white/10 hover:bg-white transition-all duration-300 shadow-xl"
    >
   ...
   <DialogContent className="sm:max-w-[500px] bg-slate-900/95 border-white/10 text-white">
    <DialogHeader>
    ...
    <div className="mt-6 space-y-4">
     ...
     <ul className="grid grid-cols-1 gap-2">
      {module.key_functionalities.map((func, i) => (
       <li key={i} className="flex items-start gap-3 text-slate-300 bg-white p-3 rounded-xl border border-white/5">
...
export const PackageCard: React.FC<PackageCardProps> = ({ type, index, allModules }) => {
 ...
 return (
  <motion.div
   ...
   whileHover={{ y: -10 }}
   className={cn(
    "relative p-8 rounded-2xl overflow-hidden border transition-all duration-500 group shadow-2xl flex flex-col",
    isPremium
  ...
  >
   {isPremium && (
    <div className="absolute top-6 right-6">
     <Badge className="bg-primary text-white hover:bg-primary border border-border px-4 py-1 rounded-full font-bold">MOST POPULAR</Badge>
    </div>
   )}
    ...
    {plan && (
     <div className="space-y-1">
      ...
      {plan.discount_amount && (
       <Badge className="bg-emerald-500/10 text-emerald-500 border border-border text-[10px] font-black uppercase">
   ...
   <div className="flex flex-col gap-3 mt-auto">
    ...
    <Dialog>
     <DialogTrigger asChild>
      <Button
       variant="ghost"
       className="w-full h-10 rounded-xl font-bold text-sm text-slate-400 hover:text-white hover:bg-white"
      >
       Explore Features
      </Button>
     </DialogTrigger>
     ...
     <div className="flex-1 overflow-y-auto px-8 pb-8 custom-scrollbar">
      ...
      {/* Limitations Notice */}
      {!isPremium && (
       <div className="mt-8 p-6 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex gap-4">
   ...
   <div className="container mx-auto px-0 relative z-10">
    ...
    >
     <motion.div
      ...
      className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-black tracking-widest uppercase mb-6"
     >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
       {features.map((f, i) => (
        <motion.div
         ...
         className="flex items-start gap-4 p-4 rounded-2xl bg-white border border-white/10 "
        >
