...
export default function ParentChapterDetailModal({ open, onOpenChange, chapterGroup }: ParentChapterDetailModalProps) {
 ...
 return (
  <Dialog open={open} onOpenChange={onOpenChange}>
   <DialogContent className="w-[95vw] sm:max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border shadow-sm bg-white p-0">
    <div className="sticky top-0 z-10 bg-white p-8 border-b border-slate-100 shrink-0">
     <DialogHeader>
    ...
    <div className="p-8 space-y-10">
     ...
     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <Card className="md:col-span-2 border border-border shadow-sm rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden">
      ...
      <Card className="border border-border shadow-sm rounded-2xl bg-white border border-slate-100 overflow-hidden">
        <CardContent className="p-6 flex flex-col justify-between h-full">
         ...
         {lessonPlan.lesson_file_url && (
          <Button variant="ghost" size="sm" className="w-full mt-4 h-10 rounded-xl bg-primary/5 text-primary font-black text-[10px] uppercase tracking-widest shadow-sm" asChild>
         ...
         {homeworkRecords.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           {homeworkRecords.map(hr => (
            <div key={hr.id} className="p-5 rounded-2xl border border-slate-100 bg-slate-50/50 flex flex-col justify-between gap-4">
              <div className="space-y-2">
               <div className="flex items-center justify-between">
                 <p className="text-sm font-black text-foreground/90 leading-tight">{hr.homework?.title}</p>
                 <Badge variant="outline" className={cn("rounded-lg border border-border text-[9px] font-black uppercase tracking-tighter", getHomeworkStatusStyles(hr.status))}>
                  {hr.status}
                 </Badge>
               </div>
