...
export default function AdmissionWorkflow({ centerId, canEdit }: { centerId: string, canEdit?: boolean }) {
 ...
 <Table>
   ...
   <Dialog open={showPublicForm} onOpenChange={setShowPublicForm}>
    <DialogContent className="w-[95vw] sm:max-w-md rounded-2xl bg-indigo-900 text-white border border-border shadow-sm">
     <DialogHeader>
     ...
     <div className="space-y-4 py-4">
       <div className="space-y-1">
        ...
        <Input value={publicFormData.studentName} onChange={e => setPublicFormData({...publicFormData, studentName: e.target.value})} className="bg-white border-white/20 text-white placeholder:text-white/40" placeholder="e.g. Rahul Sharma" />
       </div>
       <div className="space-y-1">
        <Label className="text-[10px] font-black uppercase text-indigo-300">Guardian Name</Label>
        <Input value={publicFormData.parentName} onChange={e => setPublicFormData({...publicFormData, parentName: e.target.value})} className="bg-white border-white/20 text-white placeholder:text-white/40" />
       </div>
       <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
         <Label className="text-[10px] font-black uppercase text-indigo-300">Contact Phone</Label>
         <Input value={publicFormData.parentPhone} onChange={e => setPublicFormData({...publicFormData, parentPhone: e.target.value})} className="bg-white border-white/20 text-white placeholder:text-white/40" />
        </div>
        <div className="space-y-1">
         <Label className="text-[10px] font-black uppercase text-indigo-300">Applied Grade</Label>
         <Input value={publicFormData.appliedGrade} onChange={e => setPublicFormData({...publicFormData, appliedGrade: e.target.value})} className="bg-white border-white/20 text-white placeholder:text-white/40" placeholder="e.g. 5" />
        </div>
       </div>
