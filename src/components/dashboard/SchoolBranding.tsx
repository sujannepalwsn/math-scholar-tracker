...
export default function SchoolBranding({ className, isMobileCompact }: SchoolBrandingProps) {
 ...
 return (
  <div className={cn("flex items-center justify-center gap-2 px-2 py-1 rounded-xl relative overflow-hidden transition-all duration-300", className)}>
   ...
   <div className={cn("rounded-full overflow-hidden bg-white flex items-center justify-center border border-primary/10 shrink-0 shadow-sm relative z-10", isMobileCompact ? "h-6 w-6" : "h-8 w-8")}>
    {center?.logo_url ? (
     <img
      src={center.logo_url}
      alt="Logo"
