...
export default function SubscriptionManagement() {
 ...
 return (
  <div className="space-y-8">
   ...
   {showAddPlan && (
    <Card className="rounded-2xl border border-border shadow-sm bg-white ">
     <CardContent className="p-8">
       <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-4 p-6 rounded-2xl bg-slate-50 border border-slate-100">
   ...
   <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
    {plans?.map((p: any) => (
     <Card key={p.id} className="rounded-2xl border border-border shadow-sm bg-white group hover:scale-[1.02] transition-all">
      <CardContent className="p-8">
       <div className="flex justify-between items-start mb-6">
        <Badge className="bg-amber-100 text-amber-700 border border-border font-black text-[10px] uppercase tracking-widest py-1 px-3">
         Tier: {p.name}
        </Badge>
             ...
             return (
              <div key={mod.id} className={cn(
               ...
               <div className="flex items-center justify-between mb-2">
                ...
                {isIncluded ? (
                 <Badge className="bg-emerald-500 text-white border border-border text-[9px] font-black uppercase">Included</Badge>
                ) : (
      ...
      <TableBody>
       {centerSubs?.filter((s: any) => s.status === 'Pending').map((sub: any) => (
        <TableRow key={sub.id}>
         ...
         <TableCell><Badge className="bg-primary/10 text-primary border border-border">{sub.subscription_plans?.name}</Badge></TableCell>
 ...
 <Table>
      ...
      <TableBody>
       {centerSubs?.filter((s: any) => s.status === 'Active').map((sub: any) => (
        <TableRow key={sub.id} className="hover:bg-white transition-colors">
         <TableCell className="px-8 font-bold text-slate-700">{sub.centers?.name}</TableCell>
