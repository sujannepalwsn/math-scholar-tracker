      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
       <div className="space-y-2">
        ...
        <Select value={form.centerId} onValueChange={v => setForm({...form, centerId: v})}>
         <SelectTrigger className="h-11 rounded-xl bg-white border border-border shadow-sm">
       ...
       <div className="space-y-2">
        ...
        <Input value={form.number} readOnly className="h-11 rounded-xl bg-white border border-border shadow-sm font-mono" />
       </div>
       <div className="space-y-2">
        <Label className="text-[10px] font-black uppercase text-slate-400">Total Amount</Label>
        <Input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="h-11 rounded-xl bg-white border border-border shadow-sm font-bold" />
       </div>
       <div className="space-y-2">
        <Label className="text-[10px] font-black uppercase text-slate-400">Due Date</Label>
        <Input type="date" value={form.dueDate} onChange={e => setForm({...form, dueDate: e.target.value})} className="h-11 rounded-xl bg-white border border-border shadow-sm" />
       </div>
       <div className="md:col-span-3 space-y-2">
        <Label className="text-[10px] font-black uppercase text-slate-400">Payment Description (Publicly Visible)</Label>
        <Input value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Information for the customer about the payment..." className="h-11 rounded-xl bg-white border border-border shadow-sm" />
       </div>
