 ...
 return (
  <div className="space-y-8 animate-in fade-in duration-1000">
     ...
     <div className="flex flex-col md:flex-row gap-6 md:items-center justify-between">
      ...
      <div className="w-full md:w-[250px]">
       <Select value={subjectFilter} onValueChange={setSubjectFilter}>
        <SelectTrigger className="h-12 bg-white border border-border shadow-sm focus:ring-primary/20 rounded-2xl font-bold text-xs">
         <SelectValue placeholder="All Academic Domains" />
        </SelectTrigger>
        <SelectContent className=" bg-white border border-border shadow-sm rounded-2xl">
   ...
   {isLoading ? (
    ...
    <div className="text-center py-20 bg-white rounded-2xl border border-border/20 shadow-sm">
    ...
    <div className="space-y-10">
     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {chapterRatingsBySubject.map((subjectData) => (
       <Card key={subjectData.subject} className="border border-border shadow-sm rounded-2xl bg-white border border-border/20 overflow-hidden group hover:translate-y-[-4px] transition-all duration-300">
        <CardHeader className="p-6 pb-0">
          <Badge variant="secondary" className="bg-primary/5 text-primary/70 border border-border rounded-lg text-[9px] font-black uppercase tracking-widest px-2.5 w-fit">
           {subjectData.subject}
          </Badge>
        </CardHeader>
         ...
         <TableBody>
          {chapterRatingsBySubject.flatMap(subjectData =>
           subjectData.chapters.map((record: any) => (
            <TableRow key={record.id} className="group transition-all duration-300 hover:bg-white">
