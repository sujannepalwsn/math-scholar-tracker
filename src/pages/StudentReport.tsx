...
export default function StudentReport() {
  ...
  const { data: attendanceResult, isLoading: isAttendanceLoading } = useQuery({
    queryKey: ["student-attendance", selectedStudentId, gradeFilter, dateRange, user?.center_id, user?.role, user?.id, isRestricted, attPage, attPageSize],
    queryFn: async () => {
      if (!user?.center_id) return { data: [], count: 0 };
      let query = supabase
        .from("attendance")
        .select("*, students!inner(grade)", { count: 'exact' })
  ...
  const { data: chaptersResult, isLoading: isChaptersLoading } = useQuery({
    queryKey: ["student-lesson-records-report", selectedStudentId, gradeFilter, subjectFilter, dateRange, studentIds, user?.role, user?.teacher_id, chapterPage, chapterPageSize],
    queryFn: async () => {
  ...
  const { data: testsResult, isLoading: isTestsLoading } = useQuery({
    queryKey: ["student-test-results", selectedStudentId, gradeFilter, subjectFilter, dateRange, studentIds, user?.role, user?.id, testPage, testPageSize],
    queryFn: async () => {
      let query = supabase.from("test_results").select("*, tests!inner(id, name, subject, total_marks, lesson_plan_id, questions, created_by)", { count: 'exact' })
  ...
  const { data: activitiesResult, isLoading: isActivitiesLoading } = useQuery({
    queryKey: ["student-preschool-activities-report", selectedStudentId, gradeFilter, dateRange, studentIds, user?.role, user?.id, activityPage, activityPageSize],
    queryFn: async () => {
      let query = supabase.from("student_activities").select("*, activities!inner(title, description, activity_date, photo_url, video_url, activity_type_id, created_by, activity_types(name))", { count: 'exact' })
      ...
      <div className={cn("relative group transition-all duration-700", isLoading ? "opacity-50 grayscale pointer-events-none" : "opacity-100")}>
        ...
        <div className="space-y-2">
          ...
          <Select value={gradeFilter} onValueChange={(val) => {
            setGradeFilter(val); setSelectedStudentId("none");
          ...
          <Card id="attendance-section" className="border-none shadow-strong overflow-hidden rounded-[2.5rem] bg-card/40 backdrop-blur-md border border-white/20">
            ...
            <CardContent className="p-8 space-y-8">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Records Found</p>
                  <p className="text-3xl font-black">{totalAttRows}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-green-600 uppercase tracking-widest">Present (Page)</p>
                  <p className="text-3xl font-black text-green-600">{attendanceData.filter(a => a.status === 'present').length}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Absent (Page)</p>
                  <p className="text-3xl font-black text-rose-600">{attendanceData.filter(a => a.status === 'absent').length}</p>
                </div>
                <div className="space-y-1">
              ...
              <div className="h-64 w-full bg-white/30 rounded-3xl p-6 border border-border/40 shadow-inner">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={attendanceData.slice().reverse().map(d => ({ date: format(new Date(d.date), 'MMM d'), status: d.status === 'present' ? 1 : 0 }))}>
                    <defs>
          ...
          <Card id="activities-section" className="border-none shadow-strong overflow-hidden rounded-[2.5rem] bg-card/40 backdrop-blur-md border border-border/20">
            ...
            <CardContent className="p-8">
              {isActivitiesLoading && !preschoolActivities.length ? (
                <TableSkeleton columns={3} rows={activityPageSize} />
