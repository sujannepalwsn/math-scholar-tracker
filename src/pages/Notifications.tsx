 ...
 return (
  <div className="space-y-8 animate-in fade-in duration-1000 page-enter">
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    {unreadCount > 0 && (
     <Button onClick={() => markAllReadMutation.mutate()} className="rounded-2xl shadow-sm h-12 px-6 text-sm font-black tracking-tight bg-gradient-to-r from-primary to-violet-600 hover:scale-[1.02] transition-all duration-300">
   ...
   {isLoading ? (
    <Card className="border border-border shadow-sm bg-white rounded-2xl"><CardContent className="p-12 text-center text-muted-foreground font-medium italic">Synchronizing alert registry...</CardContent></Card>
   ) : notifications.length === 0 ? (
    <Card className="border border-border shadow-sm bg-white rounded-2xl overflow-hidden border border-white/20">
    ...
    <div className="space-y-4">
     {notifications.map((n: any) => (
      <Card
       key={n.id}
       className={cn(
        "transition-all duration-300 cursor-pointer border border-border shadow-sm bg-white rounded-[1.5rem] overflow-hidden group hover:shadow-sm border border-white/20",
        !n.is_read && "bg-primary/5 border-primary/10 shadow-sm"
       )}
       ...
       <CardContent className="p-6 flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0">
         <div className="flex items-start justify-between gap-4">
          ...
          <Badge variant="secondary" className="bg-white text-slate-500 border border-border font-black text-[9px] uppercase tracking-widest px-2 py-0.5 shadow-sm shrink-0">
           {n.type}
          </Badge>
         </div>
         <div className="flex items-center gap-5 mt-4">
          ...
          <Button
           variant="ghost"
           size="icon"
           className="h-8 w-8 rounded-xl bg-white shadow-sm text-destructive hover:bg-destructive/10 ml-auto opacity-0 group-hover:opacity-100 transition-opacity"
           onClick={(e) => {
            e.stopPropagation();
