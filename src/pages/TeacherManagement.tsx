  ...
  <div className="space-y-8 animate-in fade-in duration-1000 page-enter">
     ...
     <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
      {hasFullAccess && (
       <DialogTrigger asChild>
        <Button size="lg" className="rounded-2xl shadow-sm h-12 px-6 text-sm font-black tracking-tight bg-gradient-to-r from-primary to-violet-600 hover:scale-[1.02] transition-all duration-300">
   ...
   <div className="flex flex-col lg:flex-row gap-8 min-h-[600px]">
    <div className={cn("flex-1 transition-all duration-500", selectedTeacher ? "lg:w-1/2" : "w-full")}>
     <Card className="border border-border shadow-sm overflow-hidden rounded-2xl bg-white border border-white/20">
              ...
              <TableCell className="px-6 py-4 text-right pr-6">
               <div className="flex justify-end gap-2 items-center">
                <Button
                 ...
                 size="icon"
                 className={cn(
                  "h-9 w-9 rounded-xl bg-white shadow-sm transition-all",
                ...
                {!teacher.user_id ? (
                 <Button
                  variant="default"
                  size="sm"
                  className="h-9 rounded-xl font-black uppercase text-[10px] tracking-tighter bg-emerald-600 hover:bg-emerald-700 shadow-md animate-pulse border border-border"
                 ...
                 <div className="flex items-center gap-1.5 text-[9px] font-black text-emerald-600 uppercase italic bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100 shadow-sm">
                  <ShieldCheck className="h-3.5 w-3.5" /> System Access Active
                 </div>
                )}
                <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl bg-white shadow-sm border border-slate-100 hover:bg-primary/5 transition-all" onClick={(e) => { e.stopPropagation(); handleEditClick(teacher); }}>
    ...
    {selectedTeacher && (
     <div className="lg:w-1/2 animate-in slide-in-from-right-8 duration-500">
      <Card className="rounded-2xl border border-border shadow-sm bg-white overflow-hidden sticky top-8">
       ...
       <CardContent className="p-8 space-y-8">
         ...
         <div className="flex flex-wrap gap-2">
          {getClassTeacherGrades(selectedTeacher.id).length > 0 ? (
           getClassTeacherGrades(selectedTeacher.id).map((g: string) => (
            <Badge key={g} variant="secondary" className="bg-primary/10 text-primary border border-border rounded-lg px-3 py-1 text-[10px] font-black uppercase">
             Class {g} Teacher
            </Badge>
           ))
        ...
        <div className="grid grid-cols-2 gap-3">
         ...
         {!selectedTeacher.user_id && (
          <Button
           variant="default"
           className="rounded-xl font-black uppercase text-[10px] tracking-widest h-14 bg-gradient-to-r from-indigo-600 via-primary to-violet-600 shadow-xl shadow-primary/20 border border-border hover:scale-[1.02] transition-all animate-in zoom-in duration-500"
