 ...
 return (
  <div className="space-y-8 animate-in fade-in duration-1000 page-enter">
   ...
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl shadow-sm border border-border/40 group transition-all hover:shadow-sm">
   ...
   {isLocked && !hasEditPermission && isOperationalDay && (
    <div className="flex items-center gap-3 p-4 bg-orange-50/50 border border-orange-200 rounded-2xl text-orange-700 shadow-sm animate-in slide-in-from-top-2">
   ...
   {/* Filters/Settings */}
   <Card className="border border-border shadow-sm p-6 overflow-hidden bg-white rounded-2xl">
    <div className="flex flex-wrap gap-6 items-end">
     <div className="flex-1 min-w-[200px] space-y-2">
      <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Select Date</label>
      <Popover>
       <PopoverTrigger asChild>
        <Button variant="outline" className={cn("w-full h-11 justify-start text-left font-normal bg-slate-50 border-border focus:ring-primary/20 rounded-xl", !selectedDate && "text-muted-foreground")}>
     ...
     <div className="w-[160px] space-y-2">
      <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Filter Grade</label>
      <Select value={gradeFilter} onValueChange={setGradeFilter}>
       <SelectTrigger className="h-11 bg-slate-50 border-border focus:ring-primary/20 rounded-xl">
    ...
    <CardContent>
     {filteredStudents && filteredStudents.length > 0 ? (
      <form onSubmit={handleSubmit} className="space-y-4 pt-6">
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredStudents.map((student) => (
         <div
          key={student.id}
          className={cn(
           "rounded-2xl border transition-all duration-300 p-6 flex flex-col gap-6",
           attendance[student.id]?.status === "present" ? "border-green-100 bg-green-50/30 shadow-sm" :
           attendance[student.id]?.status === "late" ? "border-yellow-100 bg-yellow-50/30 shadow-sm" :
          ...
          <div className="flex items-center justify-between">
           <div className="flex items-center gap-4">
            <div className={cn(
             ...
             attendance[student.id]?.status === "present" ? "bg-green-500 text-white shadow-sm" :
             attendance[student.id]?.status === "late" ? "bg-yellow-500 text-white shadow-sm" :
            ...
            <div className="space-y-0.5">
             ...
             <Badge variant="secondary" className="bg-primary/10 text-primary border border-border rounded-lg px-2 py-0.5 text-[10px] font-bold">
              Grade {student.grade}
             </Badge>
           ...
           <div className="flex items-center gap-2">
            <Button
             ...
             className={cn(
              ...
              attendance[student.id]?.status === "present" ? "bg-green-500 border-green-500 text-white hover:bg-green-600 shadow-sm" : "hover:bg-green-50 text-muted-foreground"
             )}
            >
            ...
            <Button
             ...
             className={cn(
              "flex-1 h-10 rounded-xl border-2 font-black text-[10px] uppercase tracking-widest transition-all gap-2",
              attendance[student.id]?.status === "late" ? "bg-yellow-500 border-yellow-500 text-white hover:bg-yellow-600 shadow-sm" : "hover:bg-yellow-50 text-muted-foreground"
             )}
            >
            ...
            <Button
             ...
             className={cn(
              "flex-1 h-10 rounded-xl border-2 font-black text-[10px] uppercase tracking-widest transition-all gap-2",
              attendance[student.id]?.status === "absent" ? "bg-red-500 border-red-500 text-white hover:bg-red-600 shadow-sm" : "hover:bg-red-50 text-muted-foreground"
             )}
            >
            ...
            <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-300">
             <div className="space-y-1.5">
              ...
              <Input
               ...
               disabled={(isLocked && !canModifyLocked) || !isOperationalDay || !canMarkAttendance}
               className="h-10 bg-white rounded-xl text-xs font-bold border border-border shadow-inner"
              />
             </div>
             <div className="space-y-1.5">
       ...
       <Button
        type="submit"
        className="w-full h-16 text-xl font-black shadow-md rounded-2xl bg-primary hover:bg-primary/90 transition-all duration-300 mt-8 text-white"
