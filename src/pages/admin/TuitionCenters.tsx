   ...
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
     <DialogTrigger asChild>
      <Button size="lg" className="rounded-[1.5rem] shadow-sm h-14 px-8 text-xs font-black tracking-widest uppercase bg-gradient-to-r from-primary to-violet-600 hover:scale-[1.05] transition-all duration-500 border border-border">
       ...
       <Table>
        ...
        <TableBody>
         {centers.map((center: any) => {
          ...
          return (
           <TableRow key={center.id} className="group transition-all duration-300 hover:bg-white">
            ...
            <TableCell className="px-6 py-4">
             {centerUser?.is_active ?
              <Badge className="bg-green-500/10 text-green-600 border border-border rounded-lg font-black uppercase text-[9px] tracking-tighter px-2 py-0.5">Operational</Badge> :
              <Badge variant="destructive" className="bg-red-500/10 text-red-600 border border-border rounded-lg font-black uppercase text-[9px] tracking-tighter px-2 py-0.5">Suspended</Badge>
             }
            </TableCell>
            <TableCell className="px-6 py-4 text-right">
             <div className="flex justify-end gap-2">
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-white shadow-sm" onClick={() => handleOpenEditDialog(center)}>
