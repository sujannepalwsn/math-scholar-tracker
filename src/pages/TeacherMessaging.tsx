...
export default function TeacherMessaging() {
  ...
  if (!user || user.role !== 'teacher' || !user.id || !user.center_id) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-muted-foreground font-medium">Please log in as a teacher to access this terminal.</p>
      </div>
    );
  }
  ...
  const { data: conversations = [], isLoading: conversationsLoading } = useQuery({
    queryKey: ["teacher-chat-conversations", user.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("chat_conversations")
        .select(`*, centers:center_id(id, name)`)
  ...
  const { data: messages = [], isLoading: messagesLoading } = useQuery({
    queryKey: ["teacher-chat-messages", selectedConversation?.id],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_messages", filter: `conversation_id=eq.${selectedConversation.id}` }, () => {
        queryClient.invalidateQueries({ queryKey: ["teacher-chat-messages", selectedConversation.id] });
      })
      .subscribe();
  ...
  const getConversationSub = (conv: any) => "Official Channel";
      ...
      <ScrollArea className="flex-1">
        {conversationsLoading ? (
          ...
          <div className="text-center py-12 px-6">
             ...
             <p className="text-muted-foreground text-xs font-medium italic">No communication history identified.</p>
          </div>
        ) : (
      ...
      <div className="p-5 border-b border-slate-100 bg-white/60 flex items-center gap-4 shrink-0">
        ...
        {selectedConversation ? (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 truncate">
                    Active Session • Administrative Liaison
                 </p>
              </div>
            </div>
            <Badge className="bg-indigo-50 text-indigo-700 border-none font-black text-[10px] px-3 py-1 uppercase tracking-widest hidden sm:flex">Station Authorized</Badge>
          </>
        ) : (
          <div className="flex items-center gap-3">
             ...
             <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Administrative Terminal Standby</p>
          </div>
        )}
      </div>
        ...
        {!selectedConversation ? (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-4 py-20 animate-in fade-in zoom-in-95 duration-700">
            ...
            <div className="text-center space-y-1">
               <p className="text-sm font-black text-slate-700 uppercase tracking-widest">Select an administrative channel</p>
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Initialize secure communication with institutional control</p>
            </div>
          </div>
        ) : messagesLoading ? (
          <div className="flex justify-center py-12"><div className="h-6 w-6 border-2 border-primary border-t-transparent animate-spin rounded-full"/></div>
          ...
          <div className="text-center py-12 px-6">
             ...
             <p className="text-muted-foreground text-sm font-medium italic">No message history identified. Liaison channel established.</p>
          </div>
        ) : (
        ...
        <div className="p-6 bg-white/60 backdrop-blur-md border-t border-slate-100 shrink-0">
          <form onSubmit={handleSendMessage} className="relative">
            <Textarea
              ...
              placeholder="Transmit institutional brief..."
          ...
          <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest text-center mt-3">
             Press Shift + Enter for new line • Transmission securely logged
          </p>
        </div>
      )}
    </div>
