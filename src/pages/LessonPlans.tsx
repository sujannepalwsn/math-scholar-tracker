 ...
 return (
  <div className="space-y-8 animate-in fade-in duration-1000 page-enter">
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    <div className="flex flex-wrap gap-3 items-center">
     <Select value={subjectFilter} onValueChange={setSubjectFilter}>
      <SelectTrigger className="w-[140px] h-10 bg-white border-muted-foreground/10 focus:ring-primary/20 rounded-xl">
     ...
     <Select value={gradeFilter} onValueChange={setGradeFilter}>
      <SelectTrigger className="w-[130px] h-10 bg-white border-muted-foreground/10 focus:ring-primary/20 rounded-xl">
     ...
     {hasActionPermission(user, 'lesson_plans', 'edit') && (
      <Button onClick={() => setIsDialogOpen(true)} size="sm" className="rounded-xl h-10 px-4 font-bold shadow-sm transition-all gap-2">
          ...
          <TableRow key={lp.id} className="group border-muted/5 hover:bg-primary/5 transition-colors">
           ...
           <TableCell>
            <div className="flex flex-col gap-1">
             <Badge variant="secondary" className="w-fit bg-primary/10 text-primary border border-border rounded-lg px-2 py-0.5 text-[9px] font-black uppercase">{lp.subject}</Badge>
           ...
           <TableCell>
            <Badge
             ...
             className={cn(
              "text-[9px] font-black uppercase tracking-widest border border-border px-2 py-1",
              lp.status === 'approved' ? "" :
           ...
           <TableCell className="text-right pr-6">
            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
             <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl bg-white shadow-sm text-primary hover:bg-primary/10" onClick={() => handleViewClick(lp)}>
              <Eye className="h-4 w-4" />
             </Button>
             {(hasActionPermission(user, 'lesson_plans', 'edit') && (isTeacher && (lp.status === 'draft' || lp.status === 'rejected'))) && (
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl bg-white shadow-sm text-primary hover:bg-primary/10" onClick={() => handleEditClick(lp)}>
               <Edit className="h-4 w-4" />
              </Button>
             )}
             {hasActionPermission(user, 'lesson_plans', 'edit') && (
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl bg-white shadow-sm text-destructive hover:bg-destructive/10" onClick={() => deleteLessonPlanMutation.mutate(lp.id)}>
   ...
   <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
    <DialogContent className="w-[98vw] sm:max-w-4xl md:max-w-5xl max-h-[90vh] md:max-h-[98vh] overflow-y-auto rounded-t-[2rem] sm:rounded-2xl p-4 sm:p-6 custom-scrollbar">
     <DialogHeader>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pr-10">
       ...
       <Button
        ...
        className="rounded-xl border-primary text-primary font-black uppercase text-[9px] sm:text-[10px] tracking-widest gap-2 shadow-sm hover:bg-primary hover:text-white transition-all"
       >
     ...
     <div className="space-y-4 sm:space-y-6 py-2 sm:py-4">
       ...
       <div className="border-2 border-slate-900 rounded-2xl sm:rounded-2xl overflow-hidden divide-y-2 md:divide-y-0 md:divide-x-2 divide-slate-900 grid grid-cols-1 md:grid-cols-2">
       ...
       <div className="space-y-1 sm:space-y-1.5">
        ...
        <Textarea value={objectives} onChange={(e) => setObjectives(e.target.value)} rows={3} placeholder="Define what students will master..." className="rounded-xl sm:rounded-2xl bg-white border-muted-foreground/10 text-xs sm:text-sm" />
       </div>
       ...
       <div className="space-y-1 sm:space-y-1.5">
        ...
        <Textarea value={warmUpReview} onChange={(e) => setWarmUpReview(e.target.value)} rows={3} placeholder="Previous lesson recap and engagement..." className="rounded-xl sm:rounded-2xl bg-white border-muted-foreground/10 text-xs sm:text-sm" />
       </div>
       ...
       <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 border-t border-muted/10">
        ...
        <Button variant="outline" onClick={() => saveLessonPlan.mutate(false)} className="flex-1 rounded-xl sm:rounded-2xl font-black uppercase text-[9px] sm:text-[10px] tracking-widest border-2 shadow-sm order-2 sm:order-2 h-10 sm:h-11" disabled={saveLessonPlan.isPending}>SAVE DRAFT</Button>
        <Button onClick={() => saveLessonPlan.mutate(true)} className="flex-[1.5] rounded-xl sm:rounded-2xl font-black uppercase text-[9px] sm:text-[10px] tracking-widest bg-gradient-to-r from-primary to-violet-600 shadow-sm hover:scale-[1.02] transition-all order-1 sm:order-3 h-10 sm:h-11" disabled={saveLessonPlan.isPending}><Send className="h-3 w-3 sm:h-4 sm:w-4 mr-2" /> SUBMIT FOR APPROVAL</Button>
       </div>
     </div>
    </DialogContent>
   </Dialog>
   ...
   <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
    <DialogContent className="w-[98vw] sm:max-w-6xl max-h-[98vh] overflow-y-auto rounded-t-none sm:rounded-2xl p-0 border border-border shadow-sm custom-scrollbar">
     <style>
      {`
       @media print {
