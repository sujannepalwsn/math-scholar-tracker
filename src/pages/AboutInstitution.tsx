 ...
 return (
  <div className={cn("space-y-8 animate-in fade-in duration-700 pb-20", isPublicView && "bg-slate-50 min-h-screen px-4 md:px-20")}>
   ...
   <div className="relative h-[400px] md:h-[600px] -mt-8 -mx-4 md:-mx-8 overflow-hidden rounded-b-[4rem] shadow-elevated">
     ...
     >
       <Badge className="mb-6 bg-white text-white border-white/40 px-6 py-2 rounded-full font-black uppercase tracking-[0.3em] text-xs">
    ...
    {canView && canEdit && (
     <div className="flex gap-4">
      <Button
       ...
       className={cn(
        "rounded-2xl h-12 px-6 font-black uppercase text-[10px] tracking-widest shadow-sm transition-all",
   ...
   <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative -mt-20 z-20 px-4 md:px-0">
    ...
    <Card className="border border-border shadow-sm rounded-2xl bg-white border border-white/40 h-full">
   ...
   <Tabs defaultValue="overview" className="space-y-8" value={activeTab} onValueChange={setActiveTab}>
    <TabsList className="bg-white border border-border/40 p-1.5 rounded-2xl h-14 shadow-sm w-full justify-start overflow-x-auto no-scrollbar">
     <TabsTrigger value="overview" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Overview</TabsTrigger>
     <TabsTrigger value="academic" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Academic Info</TabsTrigger>
     <TabsTrigger value="facilities" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Facilities</TabsTrigger>
     <TabsTrigger value="achievements" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Achievements</TabsTrigger>
     <TabsTrigger value="gallery" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Gallery</TabsTrigger>
     <TabsTrigger value="contact" className="rounded-xl px-8 font-black uppercase text-[10px] tracking-widest data-[state=active]:shadow-sm">Contact Details</TabsTrigger>
    </TabsList>
         ...
         {isEditing ? (
          <div className="space-y-2">
           ...
           <Textarea
            ...
            className="min-h-[200px] rounded-2xl bg-white"
      ...
      <div className="lg:col-span-4 space-y-8">
       <Card className="border border-border shadow-sm overflow-hidden rounded-2xl bg-white border border-border/20">
        ...
        <CardContent className="pt-6 space-y-4">
         {isEditing ? (
          <Textarea
           className="min-h-[250px] rounded-2xl bg-white"
       ...
       <Card className="border border-border shadow-sm overflow-hidden rounded-2xl bg-primary text-primary-foreground">
         ...
         <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-white/10 pb-2">
           ...
           {isEditing ? (
            <Input
             type="date"
             className="h-6 text-[10px] w-28 bg-white border border-border text-white p-1"
          ...
          <div className="flex justify-between items-center border-b border-white/10 pb-2">
           ...
           {isEditing ? (
            <Input
             className="h-6 text-[10px] w-28 bg-white border border-border text-white p-1 text-right"
    ...
    <TabsContent value="academic" className="space-y-8 outline-none animate-in fade-in slide-in-from-bottom-4 duration-500">
     <Card className="border border-border shadow-sm overflow-hidden rounded-2xl bg-white border border-border/20">
      ...
      <CardContent className="pt-6">
       {isEditing ? (
        <Textarea
         className="min-h-[400px] rounded-2xl bg-white"
      ...
      {isEditing ? (
       <div className="grid md:grid-cols-2 gap-6">
        {(Array.isArray(formData.facilities) ? formData.facilities : []).map((facility) => (
         <Card key={facility.id} className="border-2 border-dashed border-border/50 rounded-2xl p-6 space-y-4 relative group">
          <button
        ...
        {(!Array.isArray(formData.facilities) || formData.facilities.length === 0) && (
         <div className="md:col-span-2 py-20 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-muted-foreground gap-4">
       ...
       <div className="grid md:grid-cols-3 gap-6">
        {Array.isArray(center?.facilities) && (center.facilities as unknown as Facility[]).length > 0 ? (
         (center.facilities as unknown as Facility[]).map((facility) => (
          <Card key={facility.id} className="border border-border shadow-sm rounded-2xl bg-white border border-border/10 hover:shadow-sm transition-all hover:-translate-y-1">
      ...
      {isEditing ? (
       <div className="grid md:grid-cols-2 gap-6">
        {(Array.isArray(formData.achievements) ? formData.achievements : []).map((achievement) => (
         <Card key={achievement.id} className="border-2 border-dashed border-border/50 rounded-2xl p-6 space-y-4 relative group">
          <button
       ...
       <div className="grid md:grid-cols-3 gap-6">
        {Array.isArray(center?.achievements) && (center.achievements as unknown as Achievement[]).length > 0 ? (
         (center.achievements as unknown as Achievement[]).map((achievement) => (
          <Card key={achievement.id} className="border border-border shadow-sm rounded-2xl bg-white border border-border/10 hover:shadow-sm transition-all">
           <CardContent className="p-6 space-y-3">
      ...
      {isEditing ? (
       <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {(Array.isArray(formData.gallery) ? formData.gallery : []).map((item) => (
         <div key={item.id} className="relative group rounded-2xl overflow-hidden aspect-square border shadow-sm">
          ...
          <div className="absolute inset-0 bg-black/60 flex flex-col justify-end p-4 opacity-0 group-hover:opacity-100 transition-opacity">
           <Input
            ...
            className="h-8 text-[10px] bg-white border border-border text-white mb-2"
           />
           <Button
            variant="destructive"
            size="sm"
        ...
        {(!Array.isArray(formData.gallery) || formData.gallery.length === 0) && (
         <div className="col-span-full py-20 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-muted-foreground gap-4">
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {Array.isArray(center?.gallery) && (center.gallery as unknown as GalleryItem[]).length > 0 ? (
         (center.gallery as unknown as GalleryItem[]).map((item) => (
          <Card key={item.id} className="border border-border shadow-sm rounded-2xl overflow-hidden group hover:shadow-sm transition-all hover:-translate-y-1">
