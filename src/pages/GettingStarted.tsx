   ...
   {sections.map((_, i) => (
    <button
     ...
     className={cn(
      ...
      activeSection === i ? "scale-125 shadow-[0_0_10px_rgba(99,102,241,0.5)] bg-indigo-500" : "bg-white hover:bg-white"
     )}
    />
   ))}
  </div>
...
const DashboardMockup = ({ role }: { role: 'admin' | 'teacher' | 'parent' }) => {
 ...
 return (
  <motion.div
   ...
   initial="initial"
   animate="animate"
   className="relative w-full max-w-4xl aspect-[16/10] rounded-2xl border border-white/10 bg-slate-900/40 shadow-2xl overflow-hidden flex"
  >
   {/* Sidebar */}
   <div className="w-16 md:w-48 border-r border-white/5 bg-white p-4 flex flex-col gap-6">
   ...
   <div className="flex-1 p-6 md:p-8 flex flex-col gap-6 overflow-hidden">
    <div className="flex justify-between items-center">
     <div className="space-y-1">
      <div className="h-6 w-48 bg-white rounded-md" />
      <div className="h-3 w-32 bg-white rounded-md" />
     </div>
    ...
    {role === 'admin' && (
     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {[
       ...
       <div key={i} className="p-4 rounded-2xl bg-white border border-white/5 space-y-2">
        <div className={cn("w-8 h-8 rounded-lg", stat.color)} />
        <div className="text-lg font-black text-white">{stat.value}</div>
        <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{stat.label}</div>
       </div>
      ))}
      <div className="col-span-2 md:col-span-3 h-48 rounded-2xl bg-white border border-white/5 p-4">
      ...
      <div className="col-span-2 md:col-span-1 h-48 rounded-2xl bg-white border border-white/5 p-4 flex flex-col gap-3">
    ...
    {role === 'teacher' && (
     <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/5 bg-white overflow-hidden">
         ...
         {[
          ...
          <div key={i} className="flex items-center gap-4 py-2 border-b border-white/5 last:border-0">
           ...
           <div className="flex gap-2">
            <div className={cn("w-6 h-6 rounded flex items-center justify-center border", student.status === 'p' ? "bg-emerald-500/20 border-emerald-500/30 text-emerald-400" : "bg-white border-white/10 text-slate-600")}>
             <Check className="w-3 h-3" />
            </div>
            <div className={cn("w-6 h-6 rounded flex items-center justify-center border", student.status === 'a' ? "bg-rose-500/20 border-rose-500/30 text-rose-400" : "bg-white border-white/10 text-slate-600")}>
             <X className="w-3 h-3" />
            </div>
           </div>
          </div>
    ...
    {role === 'parent' && (
     <div className="flex gap-6 h-full">
      <div className="flex-1 space-y-6">
       <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-500/30 to-blue-500/30 border border-white/10 text-white">
        ...
        <div className="flex justify-between items-center">
         <div className="text-xs font-medium px-3 py-1 rounded-full bg-white">Due: June 15</div>
       ...
       <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
         <div className="p-4 rounded-2xl bg-white border border-white/5">
      ...
      <div className="w-64 flex flex-col gap-4">
       <div className="flex-1 rounded-2xl bg-white border border-white/5 p-4">
       ...
       <div className="h-32 rounded-2xl bg-white border border-white/5 p-4">
         <div className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Calendar</div>
         <div className="grid grid-cols-7 gap-1">
          {Array.from({length: 21}).map((_, i) => (
           <div key={i} className={cn("aspect-square rounded-sm", i === 12 ? "bg-primary/40" : "bg-white")} />
          ))}
         </div>
       </div>
      </div>
...
const HeroSection = ({ scrollToNext, setActiveSection }: { scrollToNext: () => void, setActiveSection: (i: number) => void }) => {
 ...
 return (
  <Section id="hero" index={0} setActiveSection={setActiveSection} noMinHeight className="bg-white p-0 md:h-[100dvh] md:min-h-[100dvh] h-auto py-10 md:py-0">
   <div className="relative w-full h-full flex items-center justify-center">
   <div className={cn(
    "z-0 transition-all duration-500",
    hasSlides
     ? "md:absolute md:inset-0 w-[85%] md:w-full aspect-[4/5] md:aspect-auto rounded-2xl md:rounded-none overflow-hidden shadow-2xl md:shadow-none"
     : "absolute inset-0"
   )}>
    {hasSlides ? (
   ...
   {!hasSlides && (
    <div className="container mx-auto relative z-10 text-center flex flex-col items-center">
     <motion.div
      ...
      className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-primary/10 text-[10px] font-black tracking-widest uppercase mb-8 bg-primary-light text-primary"
     >
     ...
     <motion.h1
      ...
      className="text-6xl md:text-8xl font-black text-slate-950 leading-[1.1] tracking-tight mb-8 max-w-4xl"
     >
     ...
     <motion.p
      ...
      className="text-xl md:text-2xl text-slate-600 font-medium max-w-2xl mb-12"
     >
...
const ProblemSection = ({ setActiveSection }: { setActiveSection: (i: number) => void }) => {
 ...
 return (
  <Section id="problems" index={1} setActiveSection={setActiveSection} className="bg-white">
   <div className="container mx-auto">
    <div className="text-center mb-20 space-y-4">
     <Badge className="bg-rose-50 text-rose-600 border-rose-100 rounded-full px-4 py-1">THE PAIN POINTS</Badge>
     <h2 className="text-4xl md:text-6xl font-black text-slate-950 tracking-tight">Managing a school <br /><span className="text-slate-400">shouldn't be this hard.</span></h2>
    </div>
    ...
    <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
     {problems.map((p, i) => (
      <motion.div
       ...
       className="p-8 rounded-2xl bg-slate-50 border border-border hover:shadow-md transition-all group"
      >
       ...
       <h3 className="text-2xl font-black text-slate-950 mb-4">{p.title}</h3>
...
const SolutionOverview = ({ setActiveSection }: { setActiveSection: (i: number) => void }) => {
 return (
  <Section id="solution" index={2} setActiveSection={setActiveSection} className="bg-slate-50/50">
   <div className="container mx-auto flex flex-col lg:flex-row items-center gap-20">
    <div className="flex-1 space-y-8">
     <Badge className="border border-border rounded-full px-4 py-1 bg-primary-light text-primary">THE SOLUTION</Badge>
     <h2 className="text-5xl md:text-7xl font-black text-slate-950 tracking-tight leading-[1.1]">
      Meet EduFlow. <br />
      <span className="text-slate-400 text-4xl md:text-5xl">Your digital command center.</span>
     </h2>
...
const FeatureSlide = ({ title, valueProp, icon: Icon, highlights, index, setActiveSection }: { title: string, valueProp: string, icon: any, highlights: string[], index: number, setActiveSection: (i: number) => void }) => {
 return (
  <Section id={`feature-${index}`} index={index} setActiveSection={setActiveSection} className={cn(index % 2 === 0 ? "bg-white" : "bg-slate-50/50")}>
    <div className="container mx-auto flex flex-col lg:flex-row items-center gap-16">
     <div className={cn("flex-1 space-y-8", index % 2 === 1 ? "lg:order-2" : "")}>
       ...
       <div className="space-y-4">
        <h2 className="text-4xl md:text-6xl font-black text-slate-950 tracking-tight">{title}</h2>
        <p className="text-2xl font-bold text-primary">{valueProp}</p>
       </div>
       <div className="grid gap-4">
        {highlights.map((h, i) => (
         <div key={i} className="flex items-start gap-4 p-4 rounded-2xl bg-white border border-border shadow-sm">
           ...
           <p className="text-slate-600 font-medium">{h}</p>
         </div>
        ))}
       </div>
     </div>
     ...
     <div className="flex-1 w-full flex justify-center">
       ...
       <div className="relative w-full max-w-lg aspect-square rounded-[3rem] bg-white border border-border p-8 flex items-center justify-center overflow-hidden shadow-sm">
        ...
        {title === "Teacher & Staff Management" && (
         <div className="relative z-10 w-full grid grid-cols-2 gap-6">
           {[1, 2, 3, 4].map(i => (
            ...
            transition={{ delay: i * 0.1 }}
            className="p-6 rounded-2xl bg-slate-900/80 border border-white/10 text-center"
            >
             <div className="w-16 h-16 rounded-full bg-white mx-auto mb-4" />
             <div className="h-3 w-20 bg-white rounded mx-auto mb-2" />
             <div className="h-2 w-12 bg-white rounded mx-auto" />
            </motion.div>
           ))}
         </div>
        )}
        ...
        {title === "Parent Portal" && (
         <div className="relative z-10 w-64 aspect-[9/19] bg-slate-950 border-[6px] border-slate-800 rounded-[3rem] p-4 flex flex-col gap-4 shadow-2xl">
           ...
           <div className="flex-1 space-y-3">
            {[1, 2, 3, 4].map(i => (
             <div key={i} className="flex items-center gap-3 p-2 rounded-xl bg-white">
               <div className="w-8 h-8 rounded-lg bg-white" />
               <div className="h-2 flex-1 bg-white rounded" />
             </div>
            ))}
           </div>
           <div className="h-12 flex justify-around items-center border-t border-white/5">
            {[1, 2, 3, 4].map(i => <div key={i} className="w-6 h-6 rounded-full bg-white" />)}
           </div>
         </div>
        )}
...
const RoleExperienceSection = ({ setActiveSection }: { setActiveSection: (i: number) => void }) => {
 ...
 return (
  <Section id="roles" index={10} setActiveSection={setActiveSection} className="bg-white">
   <div className="container mx-auto">
    <div className="text-center mb-16 space-y-4">
     <Badge className="border border-border rounded-full px-4 py-1 bg-primary-light text-primary">EXPERIENCE ROLES</Badge>
     <h2 className="text-4xl md:text-6xl font-black text-slate-950 tracking-tight">One System. <br /><span className="text-slate-400">Every Perspective.</span></h2>
    </div>

    <Tabs defaultValue="admin" className="w-full" onValueChange={setActiveTab}>
     <div className="flex justify-center mb-12">
      <TabsList className="bg-slate-50 border border-border p-1 h-14 rounded-2xl">
       <TabsTrigger value="admin" className="data-[state=active]:bg-primary data-[state=active]:text-white rounded-xl px-8 font-black uppercase text-xs transition-all">Admin</TabsTrigger>
       <TabsTrigger value="teacher" className="data-[state=active]:bg-primary data-[state=active]:text-white rounded-xl px-8 font-black uppercase text-xs transition-all">Teacher</TabsTrigger>
       <TabsTrigger value="parent" className="data-[state=active]:bg-primary data-[state=active]:text-white rounded-xl px-8 font-black uppercase text-xs transition-all">Parent</TabsTrigger>
      </TabsList>
     </div>
       ...
       <div className="flex-1 max-w-xl">
        <AnimatePresence mode="wait">
         {activeTab === 'admin' && (
          ...
          className="space-y-6"
          >
           <h3 className="text-3xl font-black text-slate-950">Complete Oversight</h3>
         ...
         {activeTab === 'teacher' && (
          ...
          className="space-y-6"
          >
           <h3 className="text-3xl font-black text-slate-950">Focus on Teaching</h3>
           <p className="text-lg text-slate-600 leading-relaxed font-medium">
         ...
         {activeTab === 'parent' && (
          ...
          className="space-y-6"
          >
           <h3 className="text-3xl font-black text-slate-950">Engaged Parenting</h3>
           <p className="text-lg text-slate-600 leading-relaxed font-medium">
...
const WorkflowSection = ({ setActiveSection }: { setActiveSection: (i: number) => void }) => {
 ...
 return (
  <Section id="workflow" index={11} setActiveSection={setActiveSection} className="bg-slate-50/50">
   <div className="container mx-auto">
    <div className="text-center mb-20 space-y-4">
     <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 rounded-full px-4 py-1">THE FLOW</Badge>
     <h2 className="text-4xl md:text-6xl font-black text-slate-950 tracking-tight">Everything Connects.</h2>
    </div>
    ...
    <div className="relative flex flex-col md:flex-row justify-between gap-8 max-w-6xl mx-auto">
      ...
      {steps.map((step, i) => (
       ...
       >
        <div
         className="w-20 h-20 rounded-2xl bg-white border border-border shadow-sm flex items-center justify-center mb-6 transition-all group-hover:bg-primary group-hover:text-white group-hover:scale-110 text-primary"
        >
...
const TrustSection = ({ setActiveSection }: { setActiveSection: (i: number) => void }) => {
 return (
  <Section id="trust" index={12} setActiveSection={setActiveSection} className="bg-white">
   <div className="container mx-auto">
    <div className="grid lg:grid-cols-2 gap-20 items-center">
     <div className="space-y-8">
      <h2 className="text-5xl md:text-7xl font-black text-slate-950 tracking-tight">Trusted by Leading Schools.</h2>
      <p className="text-xl text-slate-600 font-medium leading-relaxed">
       Join the growing network of educational institutions revolutionizing their management with EduFlow.
      </p>
      <div className="grid grid-cols-2 gap-8">
        <div>
         <div className="text-5xl font-black tracking-tight mb-2 text-primary">15,400+</div>
         <div className="text-xs font-black text-slate-500 uppercase tracking-widest">Students Managed</div>
        </div>
        <div>
         <div className="text-5xl font-black tracking-tight mb-2 text-primary">320+</div>
     ...
     <div className="grid gap-6">
       {[
        ...
        <div key={i} className="p-8 rounded-2xl bg-slate-50 border border-border relative overflow-hidden group">
         ...
         <div className="flex items-center gap-4 relative z-10">
           ...
           <div>
            <div className="font-bold text-slate-950">{t.name}</div>
...
const FinalCTASection = ({ setActiveSection, scrollToSection }: { setActiveSection: (i: number) => void, scrollToSection: (i: number) => void }) => {
 return (
  <Section id="final-cta" index={13} setActiveSection={setActiveSection} className="overflow-hidden bg-primary">
   <div className="absolute inset-0 z-0">
    ...
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] border border-white/5 rounded-full animate-pulse" />
   </div>

   <div className="container mx-auto relative z-10 text-center flex flex-col items-center text-white">
    <h2 className="text-5xl md:text-8xl font-black tracking-tight leading-[1.1] mb-8 uppercase">
    ...
    <div className="flex flex-wrap gap-6 justify-center">
     <Button asChild className="h-16 px-10 rounded-2xl text-primary text-lg font-black shadow-xl transition-all hover:scale-105 active:scale-95 bg-white hover:bg-slate-50">
      <Link to="/onboarding">Start Your 14-Day Free Trial</Link>
     </Button>
     <Button asChild variant="outline" className="h-16 px-10 rounded-2xl text-white text-lg font-black border-white/20 bg-white hover:bg-white shadow-2xl transition-all hover:scale-105 active:scale-95">
    ...
    <div className="mt-20 flex flex-col items-center gap-4">
      <div className="flex items-center gap-3 text-white">
       ...
       <span className="font-bold">2k+ schools joined last month</span>
      </div>
    </div>
   </div>
  </Section>
...
const GettingStarted = () => {
 ...
 const guideSteps = [
  {
   ...
   desc: "Set up your academic structure by defining grades and sections.",
   action: "Setup Classes",
   link: "/settings?tab=classes",
  ...
  },
  {
   ...
   desc: "Add students to your roster manually or via bulk CSV import.",
   action: "Add Students",
   link: "/register",
   icon: Users,
  ...
  },
  {
   ...
   desc: "Link educators to their respective classes and subjects.",
   action: "Manage Staff",
   link: "/teachers",
   icon: Briefcase,
  ...
  },
  {
   ...
   desc: "Start taking daily digital attendance and notifying parents.",
 ...
 return (
  <div className="bg-white min-h-screen selection:bg-primary-light selection:text-primary">
   <DotNavigation
   ...
   <nav className="fixed top-0 left-0 w-full z-[60] px-6 py-6 flex justify-between items-center pointer-events-none">
    <div className="flex items-center gap-3 pointer-events-auto cursor-pointer" onClick={() => navigate("/")}>
     <div className={cn("p-2 rounded-xl border border-primary/10 transition-colors bg-primary-light")}>
      <ShieldCheck className={cn("h-6 w-6 text-primary")} />
     </div>
     <span className="text-2xl font-black text-slate-950 tracking-tighter uppercase">Edu<span className={cn("text-primary")}>Flow</span></span>
    </div>

    <div className="pointer-events-auto flex gap-4">
      <Button asChild variant="ghost" className="text-slate-600 font-bold hover:bg-slate-50 rounded-xl px-6">
       <Link to="/login">Login</Link>
      </Button>
      <Button asChild className="bg-primary text-white font-bold rounded-xl px-6 shadow-sm">
   ...
   {/* Actionable Guide Section */}
   <Section id="guide" index={1} setActiveSection={setActiveSection} className="bg-slate-50">
    <div className="container mx-auto">
     <div className="text-center mb-16 space-y-4">
      <Badge className="bg-primary-light text-primary border border-primary/10 rounded-full px-4 py-1">GETTING STARTED</Badge>
      <h2 className="text-4xl md:text-5xl font-black text-slate-950 tracking-tight">Your Path to <span className="text-primary">Success.</span></h2>
      <p className="text-slate-600 font-medium max-w-2xl mx-auto">Follow these four simple steps to fully digitize your school administration.</p>
     </div>
     ...
     <div className="grid md:grid-cols-4 gap-6 max-w-7xl mx-auto">
      {guideSteps.map((step, i) => (
       <motion.div
        ...
        viewport={{ once: true }}
        transition={{ delay: i * 0.1 }}
        className="bg-white p-8 rounded-2xl border border-border shadow-sm hover:shadow-md transition-all group flex flex-col h-full"
       >
        <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110 shadow-sm", step.bg, step.color)}>
         <step.icon className="w-7 h-7" />
        </div>
        <h3 className="text-xl font-black text-slate-950 mb-3">{step.title}</h3>
        <p className="text-slate-600 text-sm font-medium mb-8 flex-1">{step.desc}</p>
        <Button asChild className="w-full rounded-xl font-bold bg-slate-950 hover:bg-primary transition-colors text-white">
         <Link to={step.link}>{step.action} <ArrowRight className="ml-2 h-4 w-4" /></Link>
        </Button>
       </motion.div>
      ))}
     </div>
   ...
   <footer className="bg-white py-12 px-6 border-t border-border text-center">
     <div className="flex items-center justify-center gap-3 mb-6">
      ...
      <span className="text-xl font-black text-slate-950 tracking-tighter uppercase">Edu<span className="text-primary">Flow</span></span>
     </div>
     <p className="text-slate-500 font-medium mb-8">© {new Date().getFullYear()} EduFlow Tech Solutions. All rights reserved.</p>
     <div className="flex justify-center gap-6">
      <a href="#" className="text-slate-500 hover:text-primary transition-colors">Privacy</a>
      <a href="#" className="text-slate-500 hover:text-primary transition-colors">Terms</a>
