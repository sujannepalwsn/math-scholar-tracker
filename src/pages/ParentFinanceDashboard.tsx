  ...
  <div className="space-y-8 animate-in fade-in duration-1000">
   ...
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    <div className="flex flex-wrap gap-3">
     <Button variant="ghost" onClick={() => navigate('/parent-dashboard')} className="rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white">
      <ArrowLeft className="h-4 w-4 mr-2" /> EXIT TO HUB
     </Button>
     <div className="bg-white px-4 py-2 rounded-2xl border border-border/40 shadow-sm flex items-center gap-3">
   ...
   {summary.overdue_count > 0 && (
    <div className="bg-rose-500/10 border border-rose-200 p-4 rounded-2xl flex items-center gap-4 animate-bounce-subtle">
   ...
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    {[
     ...
     <Card key={stat.title} className="border border-border shadow-sm rounded-2xl bg-white border border-border/20 group hover:translate-y-[-4px] transition-all duration-300">
     ...
     <CardHeader className="border-b border-muted/20 bg-primary/5 py-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
       ...
       <TabsList className="flex flex-nowrap overflow-x-auto w-full bg-slate-200/50 p-1 rounded-2xl">
        <TabsTrigger value="invoices" className="rounded-xl font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm px-6">Invoices</TabsTrigger>
        <TabsTrigger value="payments" className="rounded-xl font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm px-6">Payments</TabsTrigger>
       </TabsList>
      </div>
     </CardHeader>
         ...
         <TableBody>
          {invoicesLoading ? (
           ...
           invoices.map((invoice) => (
            <TableRow key={invoice.id} className="group transition-all duration-300 hover:bg-white">
             ...
             <TableCell className="px-6 py-4">
              <Badge variant="outline" className={cn("rounded-lg border border-border text-[9px] font-black uppercase tracking-tighter", getStatusStyles(invoice.status))}>
               {invoice.status}
              </Badge>
             </TableCell>
             <TableCell className="px-6 py-4 text-right">
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl bg-white shadow-sm" onClick={() => handleViewInvoice(invoice)}>
         ...
         <TableBody>
          {paymentsLoading ? (
           ...
           payments.map((payment: any) => (
            <TableRow key={payment.id} className="group transition-all duration-300 hover:bg-white">
   ...
   <Dialog open={showInvoiceDialog} onOpenChange={setShowInvoiceDialog}>
    <DialogContent className="w-[95vw] sm:max-w-2xl rounded-2xl border border-border shadow-sm bg-white ">
      ...
      <div className="space-y-8 pt-4">
       <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <div>
         ...
         <Badge className={cn("rounded-lg border border-border text-[9px] font-black uppercase tracking-tighter px-2", getStatusStyles(selectedInvoice.status))}>
