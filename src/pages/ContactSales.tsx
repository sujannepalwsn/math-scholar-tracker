...
const ContactSales = () => {
 ...
 return (
  <div className="min-h-screen bg-slate-50 selection:bg-primary-light selection:text-primary">
   {/* Navbar */}
   <nav className="sticky top-0 w-full z-50 bg-white border-b border-border px-6 py-4 flex justify-between items-center">
    <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
     <div className="p-1.5 rounded-xl bg-primary-light border border-primary/10">
    ...
    <div className="flex items-center gap-4">
      ...
      <Button asChild className="bg-primary text-white font-bold px-6 shadow-sm rounded-xl">
    ...
    <div className="grid lg:grid-cols-2 gap-16 items-start">
     ...
     >
      <div className="space-y-6">
       <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-light border border-primary/10 text-primary text-[10px] font-black tracking-widest uppercase">
        <MessageSquare className="w-3 h-3" />
        <span>Contact Sales</span>
       </div>
       <h1 className="text-4xl md:text-6xl font-black text-slate-950 leading-[1.1] tracking-tight">
        {settings?.title || "Book a Personalized Demo"}
       </h1>
       <p className="text-xl text-slate-600 font-medium leading-relaxed max-w-lg">
        {settings?.subtitle || "See why top-tier institutions trust EduFlow to power their administration and growth in Nepal."}
       </p>
      </div>

      <div className="grid gap-6">
        {benefits.map((benefit, i) => (
         <div key={i} className="flex gap-4 p-4 rounded-2xl bg-white border border-border shadow-sm transition-all hover:shadow-md hover:border-primary/20 group">
          <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-primary-light group-hover:text-primary transition-colors">
     ...
     >
      <Card className="p-8 md:p-10 rounded-2xl border border-border shadow-sm bg-white relative overflow-hidden">
        {/* Accent Gradient */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
         ...
         <Button type="submit" disabled={isSubmitting} className="w-full h-14 rounded-xl bg-primary text-white font-black text-sm uppercase tracking-widest shadow-md transition-all active:scale-95 disabled:opacity-70">
           {isSubmitting ? (
