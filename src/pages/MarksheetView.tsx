...
export default function MarksheetView() {
       ...
       <div className="space-y-2">
        ...
        <div className="relative">
         ...
         <Input
          ...
          className="pl-9 h-11 bg-white border-muted-foreground/10 focus:ring-primary/20 rounded-xl"
          value={searchQuery}
   ...
   {selectedExamId && !selectedStudentId && (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
     {filteredStudents.map((s: any) => (
      <Card
       key={s.id}
       className="border border-border shadow-sm bg-white rounded-2xl cursor-pointer hover:shadow-sm hover:scale-[1.02] transition-all duration-300 border border-white/20 group"
   ...
   {marksheetData && (
    <>
     <div className="no-print flex gap-3 justify-end">
      <Button variant="outline" onClick={handlePrint} className="rounded-xl font-bold text-xs shadow-sm bg-white h-10 px-6 border-2">
       <Printer className="h-4 w-4 mr-2" /> Print Official Copy
      </Button>
      <Button variant="outline" onClick={handlePrint} className="rounded-xl font-bold text-xs shadow-sm bg-white h-10 px-6 border-2">
       <Download className="h-4 w-4 mr-2" /> Export PDF
      </Button>
      <Button variant="outline" onClick={() => setSelectedStudentId("")} className="rounded-xl font-bold text-xs shadow-sm bg-white h-10 px-6 border-2">
       Return to Catalog
      </Button>
     </div>

     <Card className="border border-border shadow-sm rounded-2xl bg-white overflow-hidden border border-slate-200">
 ...
 <Table>
       ...
       <div className="mt-12 grid grid-cols-2 sm:grid-cols-5 gap-6 text-center">
        <div className="p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 shadow-sm">
        ...
        <div className={cn("p-6 rounded-[1.5rem] border shadow-sm", marksheetData.passed ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100")}>
