   ...
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    {canEdit && (
     <Dialog open={showMeetingFormDialog} onOpenChange={(open) => { setShowMeetingFormDialog(open); if (!open) setEditingMeeting(null); }}>
      <DialogTrigger asChild><Button size="lg" className="rounded-2xl shadow-sm h-12 px-6 text-sm font-black bg-gradient-to-r from-primary to-violet-600 hover:scale-[1.02] transition-all"><Plus className="h-5 w-5 mr-2" /> CREATE SESSION</Button></DialogTrigger>
        ...
        <TableBody>
         {meetings.map((meeting: any) => (
          <TableRow key={meeting.id} className="group transition-all duration-300">
           ...
           <TableCell><Badge variant="secondary" className="bg-slate-100 text-slate-700 border border-border font-bold uppercase text-[9px] px-2 py-0.5">{meeting.meeting_type}</Badge></TableCell>
           ...
           <TableCell className="flex gap-2">
            {canEdit && (
             <><Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl bg-white shadow-sm" onClick={() => { setEditingMeeting(meeting); setShowMeetingFormDialog(true); }}><Edit className="h-4 w-4 text-primary" /></Button>
              ...
              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl bg-white shadow-sm hover:bg-destructive/10" onClick={() => deleteMeetingMutation.mutate(meeting.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></>
            )}
           </TableCell>
          </TableRow>
         ))}
