    ...
    <>
     {canEdit && (
      <div className="flex justify-end">
       <Button onClick={() => saveMarks.mutate()} disabled={saveMarks.isPending} className="rounded-xl shadow-sm font-black uppercase text-[10px] tracking-widest h-11 px-6">
      ...
      <Table>
       <TableHeader>
        <TableRow className="hover:bg-transparent border-muted/10 bg-muted/5">
         <TableHead className="sticky left-0 bg-white z-10 min-w-[200px] pl-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Student Identity</TableHead>
         {subjects.map((s: any) => (
       ...
       <TableBody>
        {studentResults.map(({ student, totalObtained, totalFull, percentage, grade, passed, hasMarks }) => (
         <TableRow key={student.id} className="group border-muted/5 hover:bg-primary/5 transition-colors">
          <TableCell className="sticky left-0 bg-white z-10 pl-6 py-4">
          ...
          {subjects.map((subj: any) => (
           <TableCell key={subj.id} className="text-center">
            <Input
             ...
             min="0"
             max={subj.full_marks}
             className="w-20 mx-auto text-center h-9 font-bold bg-white border border-border shadow-sm rounded-xl focus:ring-primary/20"
