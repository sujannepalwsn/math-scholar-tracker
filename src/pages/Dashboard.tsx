  ...
  const cards: Record<string, React.ReactNode> = {
   "students": hasPermission(user, 'register_student') ? (
    <KPICard
     title="Students"
     value={totalStudents}
     description={totalStudents > 0 ? "Active Enrollments" : "No students yet"}
     icon={Users}
     color="indigo"
     delta={students.length > 5 ? 2.4 : 0}
     target={95}
     onClick={() => totalStudents > 0 ? navigate("/records") : navigate("/register")}
    />
   ) : null,
   "teachers": hasPermission(user, 'teacher_management') ? (
    <KPICard
     ...
     description={teachers.length > 0 ? "Active Faculty" : "No teachers yet"}
     icon={Users}
     color="blue"
   ...
   "student-attendance": hasPermission(user, 'take_attendance') ? (
    <KPICard
     ...
     description={studentAttendanceRate > 0 ? "Presence Index" : "Mark today's presence"}
     icon={CheckCircle2}
     color="green"
     trendData={attendanceTrend}
     delta={getDelta(attendanceTrend)}
     target={studentAttendanceRate}
     onClick={() => studentAttendanceRate > 0 ? navigate("/attendance-summary") : navigate(user?.role === UserRole.TEACHER ? "/teacher/take-attendance" : "/attendance")}
    />
   ) : null,
   "teacher-attendance": hasPermission(user, 'teachers_attendance') ? (
    <KPICard
     ...
     description={teacherAttendanceRate > 0 ? "Daily Logging" : "Mark faculty presence"}
     icon={Clock}
     color="orange"
   ...
   "lesson-plans": hasPermission(user, 'lesson_plans') ? (
    <KPICard
     ...
     description={upcomingLessons.length > 0 ? "Upcoming Lessons" : "Create your first plan"}
     icon={FileText}
     color="purple"
 ...
 <Table>
   ...
   <div className="flex justify-end items-center gap-4">
    ...
    <div className="relative group/search flex-1 max-w-md hidden md:block">
     ...
     <Input
      ...
      className="pl-10 h-11 rounded-2xl bg-white border-white/40 shadow-sm focus-visible:ring-primary/20 cursor-pointer group-hover:border-primary/30 transition-all"
      readOnly
      onClick={() => window.dispatchEvent(new CustomEvent('open-command-center'))}
     />
     <div className="absolute right-3 top-1/2 -translate-y-1/2 hidden lg:flex items-center gap-1 px-1.5 py-0.5 rounded-lg border bg-white text-[10px] font-black text-slate-400">
    ...
    {canEditDashboard && (
     <Button
      ...
      className={cn(
       ...
       isCustomizeMode ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground"
      )}
     >
   ...
   <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      {mainWidgetsOrder.map(id => {
       ...
       switch (id) {
        ...
        case "attendance-overview":
         content = (
          <Card className="border border-border shadow-sm bg-white rounded-2xl relative group/widget overflow-hidden transition-all hover:shadow-md">
           {isCustomizeMode && (
            <div className="absolute top-2 right-2 z-10 flex gap-1">
             <Button
              variant="secondary"
            ...
            <div className="flex gap-1 pr-12">
             {(["weekly", "monthly", "yearly", "overall"] as AttendanceRange[]).map((range) => (
              <Button
               ...
               size="sm"
               className={cn("text-xs h-7 rounded-lg capitalize", attendanceRange === range && "shadow-sm")}
           ...
           <CardContent className={cn("pt-6", !visibleWidgets[id] && "opacity-40 grayscale")}>
        ...
        case "performers":
         content = hasPermission(user, 'student_report') ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative group/widget">
           ...
           <Card className={cn("border border-border shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-all", !visibleWidgets[id] && "opacity-40 grayscale")}>
        ...
        case "teacher-status":
         content = (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative group/widget">
           ...
           <Card
            className={cn(
             "border border-border shadow-sm bg-white rounded-2xl transition-all duration-300",
             !visibleWidgets[id] && "opacity-40 grayscale",
             hasPermission(user, 'finance') && "cursor-pointer hover:shadow-md hover:border-primary/20"
            )}
        ...
        case "leave-applications":
         content = hasPermission(user, 'leave_management') ? (
          <Card className="border shadow-sm bg-card rounded-2xl overflow-hidden relative group/widget">
           {isCustomizeMode && (
            <div className="absolute top-2 right-2 z-10 flex gap-1">
             <Button
              variant="secondary"
           ...
           <CardContent className={cn("p-0", !visibleWidgets[id] && "opacity-40 grayscale")}>
            <div className="overflow-x-auto">
             <Table>
              <TableHeader>
               <TableRow className="bg-muted/5 border border-border">
        ...
        case "chapter-mastery":
         content = (
          <Card className={cn("border border-border shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-all", !visibleWidgets[id] && "opacity-40 grayscale")} key={id}>
        ...
        case "academic-efficiency":
         content = (
          <Card className={cn("border border-border shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-all", !visibleWidgets[id] && "opacity-40 grayscale")} key={id}>
           <CardHeader className="bg-primary-light/30 border-b border-primary-light/50 p-6">
            <CardTitle className="text-sm font-black uppercase tracking-widest text-primary flex items-center gap-2">
        ...
        case "academic-trends":
         content = (
          <div className="lg:col-span-2" key={id}>
            <Card className="border border-border shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-all">
             <CardHeader className="p-6 border-b border-slate-50 bg-slate-50/30"><CardTitle className="text-lg font-black flex items-center gap-2"><TrendingUp className="h-5 w-5 text-primary" /> Institutional Academic Trajectory</CardTitle></CardHeader>
         ...
         content = (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative group/widget">
           ...
           {hasPermission(user, 'preschool_activities') ? (
            <Card className={cn("border border-border shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-all", !visibleWidgets[id] && "opacity-40 grayscale")}>
             <CardHeader className="flex flex-row items-center justify-between p-6 pb-2 border-b border-slate-50 bg-slate-50/30">
              <CardTitle className="text-lg font-black flex items-center gap-3">
               <div className="p-2 rounded-xl bg-primary-light text-primary">
      ...
      <Card className="border border-border shadow-sm bg-white rounded-2xl overflow-hidden">
       <CardContent className="p-6">
        <Tabs defaultValue={hasPermission(user, 'inventory_assets') ? "library" : "transport"} className="w-full">
         <TabsList className="flex flex-nowrap w-full overflow-x-auto mb-8 bg-slate-50 border border-border p-1 rounded-xl custom-scrollbar">
          {hasPermission(user, 'inventory_assets') && (
           <TabsTrigger value="library" className="flex-1 min-w-[100px] rounded-xl font-bold text-xs uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <Book className="h-4 w-4 mr-2" /> Library
           </TabsTrigger>
          )}
          {hasPermission(user, 'transport_tracking') && (
           <TabsTrigger value="transport" className="rounded-xl font-bold text-xs uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <Bus className="h-4 w-4 mr-2" /> Transport
           </TabsTrigger>
          )}
          {hasPermission(user, 'inventory_assets') && (
           <TabsTrigger value="assets" className="rounded-xl font-bold text-xs uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm">
    ...
    <div className="lg:col-span-4 space-y-8">
     <div className={cn("space-y-8 animate-in slide-in-from-right-8 duration-1000", isCustomizeMode && "outline-dashed outline-2 outline-primary/30 outline-offset-4 rounded-2xl p-4")}>
